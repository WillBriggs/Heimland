		...
		player.setCoordsEvent(new CoordsEvent(target, new Runnable() {
			...
			public void run() {
				...
				if (target.getTemporaryAttributtes().get("clan_request_p") == player) {
					ClanWars clanWars = new ClanWars(target
							.getCurrentFriendChat(), player
							.getCurrentFriendChat());
					if (canRequest(target, player, false)) { // Make sure the
																// target can
																// still
																// challenge.
	...
	public boolean processObjectClick1(WorldObject object) {
		...
		case 28213:
			if (player.getCurrentFriendChat() == null
					|| player.getCurrentFriendChat().getClanWars() == null) {
