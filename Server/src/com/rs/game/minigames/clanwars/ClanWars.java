...
public final class ClanWars implements Serializable {
		...
		if (other != null
			...
			CoresManager.slowExecutor.submit(new Runnable() {
				...
				public void run() {
					...
					int[] newCoords = RegionBuilder.findEmptyChunkBound(width,
							height);
					...
					CoresManager.fastExecutor.scheduleAtFixedRate(
							timer = new ClanWarsTimer(ClanWars.this), 600, 600);
					enter(player);
					enter(other);
	...
	public static void enter(Player p) {
		boolean hasWar = p.getCurrentFriendChat() != null
				&& p.getCurrentFriendChat().getClanWars() != null;
		...
		if (hasWar && c.timer.isStarted() && c.isKnockOut()) {
			...
			p.getTemporaryAttributtes().put("view_prefix",
					c.firstTeam == p.getCurrentFriendChat() ? 0 : 1);
		}
		if (hasWar) {
		...
		Integer prefix = (Integer) p.getTemporaryAttributtes().get(
				"view_prefix");
