...
public class StealingCreation {
	...
	private static List<Player> redTeam = new ArrayList<Player>(),
			blueTeam = new ArrayList<Player>();// no maximum limit on it i
												// beleive
	...
	public static void enterTeamLobby(Player player, boolean inRedTeam) {
		if (!canEnter(player, inRedTeam)) {
			...
			CoresManager.fastExecutor.scheduleAtFixedRate(
					lobbyTask = new LobbyTimer(), 0, 60000);
		player.getControlerManager().startControler("StealingCreationsLobby",
				inRedTeam);
	}
	...
	public static void passToGame() {
		...
		for (Player player : blueTeam) {
			player.getControlerManager().startControler(
					"StealingCreationsGame", false);
		}
	}

	public static boolean proccessKilnItems(Player player, int componentId,
			int index, int itemId, int amount) {
	...
	public static boolean checkSkillRequriments(Player player,
			int requestedSkill, int index) {
		int level = getLevelForIndex(index);
		if (player.getSkills().getLevel(requestedSkill) <= level) {
			player.getPackets().sendGameMessage(
					"You dont have the requried "
	...
	public static void startDynamicSkill(Player player, WorldObject object,
			Animation animation, int baseId, int objectIndex) {
	...
	public static void updateTeamInterface(Player player, boolean inRedTeam,
		...
		for (int i = 33; i < 34; i++)
			player.getPackets().sendIComponentText(
					...
					i,
					""
							+ (hidden ? 5 - (inRedTeam ? redTeam.size()
									: blueTeam.size()) : inRedTeam ? redTeam
									.size() : blueTeam.size()));
	}
	...
	private static boolean canEnter(Player player, boolean inRedTeam) {
		...
		if ((skillTotal + combatTotal) > (otherSkillTotal + otherCombatTotal)) {
			...
			return false;
				...
				|| player.getInventory().getItemsContainerSize() != 0
