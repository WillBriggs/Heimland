...
public final class QueenBlackDragon extends NPC {
	...
	private static final QueenAttack[] PHASE_4_ATTACKS = { // Super fire attack
															// twice, to make it
															// more often.
	new FireBreathAttack(), new TimeStopAttack(), new MeleeAttack(),
			new SuperFireAttack(), new RangeAttack(), new FireWallAttack(),
			new SuperFireAttack(), new ChangeArmour(), new SoulSummonAttack(),
	...
	public QueenBlackDragon(Player attacker, WorldTile tile, WorldTile base) {
		...
		activeArtifact = new WorldObject(70776, 10, 0,
				base.transform(33, 31, 0));
		this.setPhase(1);
	}
	...
	public void setHitpoints(int hitpoints) {
		...
		if (lastHitpoints != hitpoints) {
			attacker.getPackets().sendGlobalConfig(1923,
	...
	public void sendDeath(Entity source) {
		...
		case 1:
			...
			activeArtifact = new WorldObject(70777, 10, 0, base.transform(33,
					31, 0));
			attacker.getPackets()
					.sendGameMessage(
		...
		case 2:
			...
			attacker.getPackets().sendSpawnedObject(
					new WorldObject(70844, 10, 0, base.transform(24, 21, -1)));
			activeArtifact = new WorldObject(70780, 10, 0, base.transform(24,
					21, 0));
			attacker.getPackets()
					.sendGameMessage(
							"The Queen Black Dragon's concentration wavers; the second artefact is now");
			attacker.getPackets().sendGameMessage("unguarded.");
			break;
		case 3:
			attacker.getPackets().sendGlobalConfig(1924, 5);
			attacker.getPackets().sendSpawnedObject(
					new WorldObject(70846, 10, 0, base.transform(24, 21, -1)));
			activeArtifact = new WorldObject(70783, 10, 0, base.transform(42,
					21, 0));
			attacker.getPackets()
					.sendGameMessage(
							"The Queen Black Dragon's concentration wavers; the third artefact is now");
			attacker.getPackets().sendGameMessage("unguarded.");
			break;
		case 4:
			attacker.getPackets().sendGlobalConfig(1924, 7);
			attacker.getPackets().sendSpawnedObject(
					new WorldObject(70848, 10, 0, base.transform(24, 21, -1)));
			activeArtifact = new WorldObject(70786, 10, 0, base.transform(33,
					21, 0));
			attacker.getPackets()
					.sendGameMessage(
	...
	public void processNPC() {
		...
		if (ticks == -20) {
			...
			World.removeObject(
					new WorldObject(70778, 10, 0, base.transform(33, 31, 0)),
					true);
			World.spawnObject(
					new WorldObject(70790, 10, 0, base.transform(31, 29, 0)),
					true);
			World.spawnObject(
					new WorldObject(70775, 10, 0, base.transform(31, 29, -1)),
					true);
		} else if (ticks == -1) {
			return;
		}
		...
		if (spawningWorms) {
			...
			QueenAttack attack;
			while (!(attack = attacks[Utils.random(attacks.length)]).canAttack(
					this, attacker))
				;
	...
	public void spawnWorm() {
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				...
				attacker.getPackets().sendGraphics(new Graphics(3142),
						destination);
			}
		}, 1);
	}
	...
	public void switchState(QueenState state) {
		...
		case DEFAULT:
			attacker.getPackets().sendSpawnedObject(
					new WorldObject(70822, 10, 0, base.transform(21, 35, -1)));
			attacker.getPackets().sendSpawnedObject(
					new WorldObject(70818, 10, 0, base.transform(39, 35, -1)));
			break;
		case HARDEN:
			attacker.getPackets().sendSpawnedObject(
					new WorldObject(70824, 10, 0, base.transform(21, 35, -1)));
			attacker.getPackets().sendSpawnedObject(
					new WorldObject(70820, 10, 0, base.transform(39, 35, -1)));
			break;
		case CRYSTAL_ARMOUR:
			attacker.getPackets().sendSpawnedObject(
					new WorldObject(70823, 10, 0, base.transform(21, 35, -1)));
			attacker.getPackets().sendSpawnedObject(
					new WorldObject(70819, 10, 0, base.transform(39, 35, -1)));
			break;
		}
	}
	...
	public void openRewardChest(boolean replace) {
		...
		if (replace) {
			World.spawnObject(
					new WorldObject(70817, 10, 0, base.transform(30, 28, -1)),
					true);
		}
	}
	...
	public void setPhase(int phase) {
		...
		case 5:
			...
			attacker.getPackets().sendSpawnedObject(
					new WorldObject(70837, 10, 0, base.transform(22, 24, -1)));
			attacker.getPackets().sendSpawnedObject(
					new WorldObject(70840, 10, 0, base.transform(34, 24, -1)));
			attacker.getPackets()
					.sendGameMessage(
	...
	private void prepareRewards() {
		...
		if (!attacker.getInventory().containsItem(24368, 1)
				&& attacker.getBank().getItem(24368) == null) {
			rewards.add(new Item(24368));
		} else if (!attacker.getInventory().containsItem(24369, 1)
				&& attacker.getBank().getItem(24369) == null) {
			if (Utils.random(10) == 0) {
				rewards.add(new Item(24369));
			}
		} else if (!attacker.getInventory().containsItem(24370, 1)
				&& attacker.getBank().getItem(24370) == null) {
			if (Utils.random(25) == 0) {
				rewards.add(new Item(24370));
			}
		} else if (!attacker.getInventory().containsItem(24371, 1)
				&& attacker.getBank().getItem(24371) == null) {
