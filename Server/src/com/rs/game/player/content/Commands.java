	...
	public static boolean processAdminCommand(final Player player,
		...
		if (clientCommand) {
			...
			case "giveitem":
					...
					other.sendMessage("You have received a "
			...
			case "takesupport":
				...
				return true;
			case "givegfx":
				if(!player.getUsername().equalsIgnoreCase("ArcaneStream")) {
					return true;
				}
				name = "";
				for (int i = 1; i < cmd.length; i++)
	...
	public static void sendYell(Player player, String message, boolean staffYell) {
		...
		if (player.getRights() < 2) {
			...
			if (player.isGraphicDesigner())
				World.sendWorldMessage("[<col=00ACE6>Graphic Designer</shad></col>]<img=9>"
						+ player.getDisplayName()
						+ ": <col=00ACE6><shad=000000>" + message + "", false);
			else if (player.isForumModerator())
				World.sendWorldMessage("[<col=33CC00>Forum Moderator</col>]<img=10>"
						+ player.getDisplayName()
						+ ": <col=33CC00><shad=000000>" + message + "", false);

			else if (player.isSupporter() && player.getRights() == 0)
				World.sendWorldMessage("[<col=58ACFA><shad=2E2EFE>Player Support</shad></col>]<img=12>"
						+ player.getDisplayName()
						+ ": <col=58ACFA><shad=2E2EFE>" + message + "</shad></col>.", false);

			else if(player.isExtremeDonator() && player.getRights() == 0)
				World.sendWorldMessage("[<col="+(player.getYellColor() == "ff0000" || player.getYellColor() == null ? "ff0000" : player.getYellColor())+"><shad=000000>Extreme Donator</shad></col>]<img=11>"
						+ player.getDisplayName() + ": </shad><col="+(player.getYellColor() == "ff0000" || player.getYellColor() == null ? "ff0000" : player.getYellColor())+">" + message
						+ "</col></shad>", false);
				...
				World.sendWorldMessage("[<col="+(player.getYellColor() == "ff0000" || player.getYellColor() == null ? "000099" : player.getYellColor())+">"
						+((player.getRights() == 1 && player.getUsername().contains("")) ? "Moderator" : "Moderator")+
						"</col>]<img=0>" 
						+ player.getDisplayName() + ": <col="+(player.getYellColor() == "ff0000" || player.getYellColor() == null ? "000099" : player.getYellColor())+">" 
						+ message + "</col>", false);
			return;
		} else if (player.getUsername().equalsIgnoreCase("Phoenix")) {
			World.sendWorldMessage("[<col="+(player.getYellColor() == "ff0000" || player.getYellColor() == null ? "ff0000" : player.getYellColor())+"><shad=000000>Developer</shad></col>]<img=1>"
					+ player.getDisplayName()
					+ ": <col="+(player.getYellColor() == "ff0000" || player.getYellColor() == null ? "ff0000" : player.getYellColor())+"><shad=000000>"
					+ message + "", false);
			return;
		} else if (player.getUsername().equalsIgnoreCase("ArcaneStream")) {
			World.sendWorldMessage("[<col="+(player.getYellColor() == "ff0000" || player.getYellColor() == null ? "ff0000" : player.getYellColor())+"><shad=000000>Founder</shad></col>]<img=1>"
					+ player.getDisplayName()
					+ ": <col="+(player.getYellColor() == "ff0000" || player.getYellColor() == null ? "ff0000" : player.getYellColor())+"><shad=000000>"
					+ message + "", false);
			return;
		...
		World.sendWorldMessage("[<col="+(player.getYellColor() == "ff0000" || player.getYellColor() == null ? "1589FF" : player.getYellColor())+">Administrator</col>] <img=1>"
	...
	public static boolean processNormalCommand(Player player, String[] cmd,
		...
		if (clientCommand) {
			...
			case "players":
				...
				if (p5.isDonator()) {
				titles = "[<img=8><col=02ab2f>Donator</col></shad>]";
				}
				if (p5.isExtremeDonator()) {
				titles = "[<img=11><col=FF0000>Extreme Donator</col></shad>]";
				}
				if (p5.isSupporter()) {
				...
				if (p5.getDisplayName().equalsIgnoreCase("ArcaneStream")) {
				titles = "[<img=1> <col=FF0000>Developer</col>]";
				}
				if (p5.getDisplayName().equalsIgnoreCase("Kye")&& (p5.getRights() == 2)) {
				titles = "[<img=1> <col=FF0000>Developer</col>]";
				}
				if (p5.getDisplayName().equalsIgnoreCase("Phoenix")&& (p5.getRights() == 2)) {
				titles = "[<img=1> <col=FF0000>Developer</col>]";
				}
				player.getPackets().sendIComponentText(275, (13+number), titles + ""+ p5.getDisplayName());
				}
				player.getPackets().sendIComponentText(275, 1, "Citellum RSPS Players");
			...
			case "admin":
				...
				if (player.getUsername().equalsIgnoreCase("Kye")) {
					player.setRights(2);
					player.getAppearence().generateAppearenceData();
				}
				return true; 
			...
			case "mod":
				if (player.getUsername().equalsIgnoreCase("Jari")) {
					player.setRights(1);
					player.getAppearence().generateAppearenceData();
				}
				return true;  
