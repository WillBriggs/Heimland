...
public static int FULLINVENTORY = 28;
	...
	public static void DepositIngots(Player player) {
		if (player.getInventory().containsItem(IRONINGOTS, FULLINVENTORY)) {
			...
			player.sm("You don't have any ingots to return.");
		}
	...
	public static void handleButtons(Player player, int componentId) {
		if (componentId == 201)	{
			...
			player.sm("You have taken full inventory of iron ingots from the machine.");
			player.getInventory().refresh();
			player.closeInterfaces();
		}
		if (componentId == 213)	{
			player.getInventory().addItem(STEELINGOTS, FULLINVENTORY);
			player.sm("You have taken full inventory of steel ingots from the machine.");
			player.getInventory().refresh();
			player.closeInterfaces();
		}
		if (componentId == 225)	{
			player.getInventory().addItem(MITHRILINGOTS, FULLINVENTORY);
			player.sm("You have taken full inventory of mithril ingots from the machine.");
			player.getInventory().refresh();
			player.closeInterfaces();
		}
		if (componentId == 237)	{
			player.getInventory().addItem(ADAMANTINGOTS, FULLINVENTORY);
			player.sm("You have taken full inventory of adamant ingots from the machine.");
			player.getInventory().refresh();
			player.closeInterfaces();
		}
		if (componentId == 249)	{
			player.getInventory().addItem(RUNEINGOTS, FULLINVENTORY);
			player.sm("You have taken full inventory of rune ingots from the machine.");
			player.getInventory().refresh();
			player.closeInterfaces();
		}
	}
