...
public class AuraManager implements Serializable {
	...
	public void activate() {
		...
		if (toId != -1) {
			...
			if (activation != 0) {
				player.getPackets().sendGameMessage("Your aura is already active.");
				return;
			}
			if (Utils.currentTimeMillis() <= getCoolDown(item.getId())) {
				player.getPackets().sendGameMessage("Your aura is cooling down.");
				return;
			}
			int tier = getTier(item.getId());
			activation = Utils.currentTimeMillis()
					+ getActivationTime(item.getId()) * 1000;
			cooldowns.put(item.getId(), activation + getCooldown(item.getId())
					* 1000);
