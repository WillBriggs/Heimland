...
public class PortalTeleport extends Dialogue {
	...
	public void start() {
		...
		sendOptionsDialogue("Pick a teleport type", "Training locations",
				"Minigame locations", "Boss Locations", "Activities",
				"Next Page");
	...
	public void run(int interfaceId, int componentId) {
		if (stage == 1) {
			if (componentId == OPTION_1) {
				sendOptionsDialogue("Where would you like to go?",
						"Jastiszo: Rock Crabs", "Harmony: Dead Zombie Island", "Canifis Ghous",
						"Relleka: Rock Crabs", "");
				stage = 2;
			} else if (componentId == OPTION_2) {

				sendOptionsDialogue("Where would you like to go?", "Barrows Brothers",
						"Clan Wars", "Duel Arena", "Castle Wars", "Next page");
				stage = 3;
			} else if (componentId == OPTION_3) {
				sendOptionsDialogue("Boss Teleports | Page: 1", "Godwars: Nex",
						"Godwars: Bandos", "Godwars: Saradomin.",
						"Tormented Demons", "Next Page");
				stage = 69;
			...
			if (componentId == OPTION_1)
				...
				sendOptionsDialogue("Boss Teleports | Page: 2",
						"Corporeal Beast", "King Black Dragon",
						"Queen Black Dragon", "Godwars: Armadyl", "Next page");
			}
		} else if (stage == 70) {
			if (componentId == OPTION_1) {
				...
				sendOptionsDialogue("Boss Teleports | Page: 3",
						"Relleka Woods: Hati", "Wilderness: Wildywyrms", "Icy Caves: Glacors", "Avatar of Destruction");
			}
		} else if (stage == 77) {
			if (componentId == OPTION_1) {
			...
			if (componentId == OPTION_1) {
				Magic.sendNormalTeleportSpell(player, 0, 0, new WorldTile(3561,
						3293, 0));
			} else if (componentId == OPTION_2) {
			...
			if (componentId == OPTION_1) {
				...
				Magic.sendNormalTeleportSpell(player, 0, 0, new WorldTile(3358,
						6112, 0));
			} else if (componentId == OPTION_5) {
				sendOptionsDialogue("Where would you like to go?",
		...
		Magic.sendNormalTeleportSpell(player, 0, 0, new WorldTile(1740, 5311, 0));
			...
			if (componentId == OPTION_1) {
				...
				sendOptionsDialogue("Where would you like to go?", "Zombies",
						"Recipe for Disaster", "Beggening Options");
				stage = 50;
			}
		}
