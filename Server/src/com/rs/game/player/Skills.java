...
public final class Skills implements Serializable {
	...
	public void addXp(int skill, double exp) {
		...
		if (skill != ATTACK && skill != DEFENCE && skill != STRENGTH
			...
			exp *= player.isExtremeDonator() ? Settings.XP_RATE + 5 : player.isDonator() ? Settings.XP_RATE + 2 : Settings.XP_RATE;
		if (player.getAuraManager().usingWisdom())
			exp *= 1.025;
		if (skill == ATTACK || skill == DEFENCE || skill == STRENGTH || skill == RANGE || skill == MAGIC || skill == THIEVING)
            exp *= 20*Settings.XP_RATE;
		if (skill == AGILITY || skill == FLETCHING)
            exp *= 2*Settings.XP_RATE;
		if (skill == SUMMONING || skill == MINING)
        exp *= 1*Settings.XP_RATE;
		if (skill == PRAYER)
            exp *= .30*Settings.XP_RATE;
		if (skill == RUNECRAFTING)
            exp *= .20*Settings.XP_RATE;
		if (skill == THIEVING)
			exp *= .04*Settings.XP_RATE;
		if (skill == HERBLORE)
            exp *= 1*Settings.XP_RATE;
        if (skill == HITPOINTS)
            exp *= 30*Settings.XP_RATE;
        if (skill == SLAYER)
            exp *= 5*Settings.XP_RATE;
