...
public class FightCaves extends Controler {
	...
	public static void enterFightCaves(Player player) {
		...
		player.getControlerManager().startControler("FightCavesControler", 1); // start
																				// at
																				// wave
																				// 1
	}
		...
		if (type == 0 || type == 2)
			...
			if (type == 1 || type == 4) {
				...
				if (type == 4) {
					...
					if (!player.getInventory().addItem(6570, 1) && player.getInventory().addItem(23659, 1)) {
						World.addGroundItem(new Item(6570, 1), new WorldTile(
								player), player, true, 180, true);
										World.addGroundItem(new Item(23659, 1), new WorldTile(
								player), player, true, 180, true);
