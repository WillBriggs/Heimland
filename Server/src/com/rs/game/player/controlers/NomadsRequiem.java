...
public class NomadsRequiem extends Controler {
	...
	public boolean sendDeath() {
		if (currentPart == DungeonPart.THRONE) {
			...
			List<Integer> indexes = World.getRegion(player.getRegionId())
					.getNPCsIndexes();
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				if (stage == 0) {
					...
					WorldTile walk = new WorldTile(getBaseX() + 35,
							getBaseY() + 11, 0);
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				if (stage == 1) {
					...
					WorldTile throne = new WorldTile(getBaseX() + 16,
							getBaseY() + 20, 0);
					Nomad nomad = (Nomad) World.spawnNPC(8528, throne, -1,
							true, true);
