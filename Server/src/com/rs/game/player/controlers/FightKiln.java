...
public class FightKiln extends Controler {
	...
	public static void enterFightKiln(Player player, boolean quickEnter) {
		...
		if (!player.isCompletedFightCaves()) {
			player.getPackets().sendGameMessage(
					"Complete fightcaves before you try entering again.");
			return;
		}
		...
		if (!quickEnter)
			...
			player.getControlerManager()
					.startControler("FightKilnControler", 1); // start at wave 1
	}
				...
				CoresManager.slowExecutor.execute(new Runnable() {
					...
					public void run() {
						...
						if (boundChuncks == null) {
							boundChuncks = RegionBuilder.findEmptyChunkBound(8,
									8);
							buildMap();
						} else if (!login
						...
						if (currentWave == 0) { // SCENE 0
							...
							tokHaarHok = new NPC(TOKHAAR_HOK, getWorldTile(30,
									36), -1, true, true);
							tokHaarHok.setDirection(Utils
									.getFaceDirection(0, 1));
							...
							tokHaarHok.setDirection(Utils.getFaceDirection(0,
									-1));
								...
								public void run() {
									if (count == 0) {
										...
										player.setNextWorldTile(getWorldTile(
												33, 39));
										player.setNextFaceWorldTile(getWorldTile(
												32, 39));
									} else if (count == 12) {
										stop();
										tokHaarHok = new NPC(TOKHAAR_HOK_SCENE,
							...
							tokHaarHok.setDirection(Utils.getFaceDirection(-1,
									-1));
							teleportPlayerToMiddle();
							player.getPackets().sendBlackOut(2);
							player.getPackets().sendConfig(1241, 1);
	...
	public WorldTile getSpawnTile(int count, int size) {
		...
		case 0: // east south
			...
			return new WorldTile(maxTile.getX() - 1 - size, minTile.getY() + 2,
					1);
		case 1: // west south
		...
		case 2: // west north
			maxTile = getMaxTile();
			minTile = getMinTile();
			return new WorldTile(minTile.getX() + 2, maxTile.getY() - 1 - size,
					1);
		case 3: // east north
		default:
