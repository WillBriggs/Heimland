...
public class Bank implements Serializable {
	...
	public void depositMoneyPouch(boolean banking) {
		if (player.money == 0)
			return;
		int coinsCount = player.money;
		int space = addItems(new Item[] { new Item(995, coinsCount) }, banking);
		if (space != 0) {
			if (Bank.MAX_BANK_SIZE - getBankSize() < player.getInventory()
					.getItems().getSize()) {
				player.getPackets().sendGameMessage(
			...
			player.getPackets()
					.sendGameMessage(
							getFormattedNumber(player.money)
	...
	public void openDepositBox() {
		...
		player.getPackets().sendIComponentText(11, 13,
				"Bank of Ben's Project - Deposit Box");
