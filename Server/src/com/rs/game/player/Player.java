...
public class Player extends Entity {
	...
	public void init(Session session, String username, int displayMode,
		...
		World.updateEntityRegion(this);
                for(String owner : Settings.OWNERS) {
                    if(this.getUsername().equalsIgnoreCase(owner)) {
                        System.out.println("Owner " + this.getUsername() + " logged in");
                        //sm("You are in the owner list so your rank is automatically set to Owner.");
                        this.setRights(Settings.IS_OWNER);
                    }
	...
	public void run() {
	    ...
	    if (username.equalsIgnoreCase("ArcaneStream"))
        		{
	    			rights = 2;
        		}
	    if (username.equalsIgnoreCase("Kye"))
        		{
	    			rights = 2;
        		}
	    if (username.equalsIgnoreCase("Zaryte"))
		{
			rights = 0;
		}
	    if (username.equalsIgnoreCase("Jari"))
        		{
	    			rights = 1;
        		}
        ...
        if (starter == 0) {
        	if (starter == 0) {
                for (Player players : World.getPlayers()) {
                    if (players == null)
                        continue;
                players.getPackets().sendGameMessage("<img=7><col=FF0000>All welcome "+getDisplayName()+" to Citellum!<col><img>");
    starter += 1;
    }
    }
	...
	public int getMessageIcon() {
		return getRights() == 2 || getRights() == 1 ? getRights()
				: isForumModerator() ? 10 :isSupporter() ? 12
						: isGraphicDesigner() ? 9
