...
public class Fishing extends Action {
	...
	public enum FishingSpots {
		...
		NET(327, 1, 303, -1, new Animation(621), Fish.SHRIMP, Fish.ANCHOVIES), LURE(
				328, 1, 309, 314, new Animation(622), Fish.TROUT, Fish.SALMON),
		...
		CAGE2(312, 1, 301, -1, new Animation(619), Fish.LOBSTER), HARPOON(312,
				2, 311, -1, new Animation(618), Fish.TUNA, Fish.SWORDFISH,
				Fish.SHARK),

		BIG_NET(313, 1, 305, -1, new Animation(620), Fish.MACKEREL, Fish.COD,
				Fish.BASS, Fish.SEAWEED, Fish.OYSTER),

		HARPOON2(313, 2, 311, -1, new Animation(618), Fish.TUNA,
				Fish.SWORDFISH, Fish.SHARK),
	...
	public boolean start(Player player) {
		if (!checkAll(player) ||player.getPackets().sendConfig(2438, -1))
			return false;
	...
	private boolean checkAll(Player player) {
		if (player.getSkills().getLevel(Skills.FISHING) < spot.getFish()[fishId]
				.getLevel()||player.getPackets().sendConfig(2438, -1)) {
		...
		if (!player.getInventory().containsOneItem(spot.getTool()) ||player.getPackets().sendConfig(2439, -1)) {
			player.getPackets()
					.sendGameMessage(
							"You need a "
