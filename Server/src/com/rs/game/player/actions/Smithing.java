...
public class Smithing extends Action {
	...
	public enum ForgingBar {
		...
		IRON_INGOT(20632, 1, new Item[] { new Item(20572, 1), new Item(20577, 1),
				new Item(20582, 1), new Item(20587, 1), new Item(20597, 1),
				new Item(20602, 1), new Item(20607, 1), new Item(20612, 1),
		...
		public static ForgingBar getBar(Player player) {
			...
			for (ForgingBar bar : bars.values()) {
				if (smithLevel < bar.getLevel()
						|| !player.getInventory().containsItem(bar.getBarId(),
								1))
					continue;
				return bar;
			}
	...
	public boolean start(Player player) {
		if ((bar = (ForgingBar) player.getTemporaryAttributtes().get(
				"SmithingBar")) == null) {
			return false;
		}
	...
	public static class ForgingInterface {
		...
		public static int getActualAmount(int levelRequired, ForgingBar bar,
			...
			if (level >= 0 && level <= 4) {
				...
				if (name.contains("knife") || name.contains("limb")
						|| name.contains("studs")) {
					return 1;
				}
				return 2;
